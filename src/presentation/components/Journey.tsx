        ...
        <div className="space-y-4">
          {steps.map((s, i) => (
            <ScrollReveal key={s.num} delay={i * 60} variant="up">
              <div className="group relative overflow-hidden rounded-2xl border border-line bg-bg-soft transition hover:border-line-strong">
                <div className="grid grid-cols-1 gap-8 p-8 lg:grid-cols-12 lg:gap-12 lg:p-10">
                  ...
                  <div className="lg:col-span-4">
                    <div className="flex items-baseline gap-3">
                      ...
                      <span className="font-mono text-[11px] text-ink-faint">· {s.time}</span>
                    </div>
                    <h3 className="mt-3 font-serif text-3xl leading-tight">
                      {s.title}
                      <span className="ml-1 text-ink-faint">.</span>
                    </h3>
                    <p className="mt-4 text-[14px] leading-[1.7] text-ink-dim">{s.body}</p>
                  </div>
          ...
          <div className="flex items-center justify-between rounded-2xl border border-line bg-bg-soft p-8">
            <div>
              ...
              <div className="mt-2 font-serif text-4xl text-ink">
                ~2.1ms
                <span className="ml-3 font-mono text-xs text-ink-dim">from connect to commit</span>
              </div>
            </div>
            <div className="hidden text-right md:block">
              ...
              <div className="mt-2 font-serif text-4xl gradient-text">47 years</div>
            </div>
          </div>
        </ScrollReveal>
      </div>
          ...
          <div key={i}>
            {isComment ? (
              ...
              <span>
                {line
                  .split(
                    /(\b(?:import|from|const|await|async|function|return|true|false)\b|"[^"]*")/g
                  )
                  .map((part, j) => {
                    if (
                      /\b(import|from|const|await|async|function|return|true|false)\b/.test(part)
                    ) {
                      return (
                    ...
                    if (/^"[^"]*"$/.test(part)) {
                      return (
                        <span key={j} className="text-accent-warm">
                          {part}
                        </span>
                      );
                    }
