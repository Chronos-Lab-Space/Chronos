...
| **Capability registry** | `runTaskGraph` executes a full planner graph but has **no production caller** — it is exercised by tests only, so `roadmap.build` and the rest of the launch graph never run outside them. Dispatched in production: `simulation.execute`, `outcome.evaluate` (via `core/runtime`) and `plan` (via `agent-os`). Registered but never dispatched: coding, knowledge, research, memory. Product UI “workloads” under `capabilities.ts` are **demo metadata**, not the OS registry. |
